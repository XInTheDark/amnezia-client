...
constexpr int HANDSHAKE_TIMEOUT_MSEC = 45000;
...
{
    if ((config.m_hopType == InterfaceConfig::MultiHopExit) || (config.m_hopType == InterfaceConfig::SingleHop)) {
...
// static
bool Daemon::parseStringList(const QJsonObject &obj, const QString &name, QStringList &list)
{
...
#define GETVALUE(name, where, jsontype)                                                                                \
    ...
    if (config.m_deviceIpv4Address.isNull() && config.m_deviceIpv6Address.isNull()) {
    ...
    if (config.m_serverIpv4AddrIn.isNull() && config.m_serverIpv6AddrIn.isNull()) {
    ...
    if (!obj.contains("hopType")) {
        ...
        config.m_hopType = InterfaceConfig::HopType(meta.keyToValue(vdata.constData(), &okay));
        if (!okay) {
    ...
    if (!obj.contains(JSON_ALLOWEDIPADDRESSRANGES)) {
        logger.error() << JSON_ALLOWEDIPADDRESSRANGES << "missing in the jsonconfig input";
        return false;
    } else {
        ...
        for (const QJsonValue &i : array) {
            if (!i.isObject()) {
                logger.error() << JSON_ALLOWEDIPADDRESSRANGES << "must contain only objects";
                return false;
            }
            ...
            if (!address.isString()) {
                logger.error() << JSON_ALLOWEDIPADDRESSRANGES << "objects must have a string address";
                return false;
            }

            QJsonValue range = ipObj.value("range");
            if (!range.isDouble()) {
                logger.error() << JSON_ALLOWEDIPADDRESSRANGES << "object must have a numberic range";
                return false;
            }

            QJsonValue isIpv6 = ipObj.value("isIpv6");
            if (!isIpv6.isBool()) {
                logger.error() << JSON_ALLOWEDIPADDRESSRANGES << "object must have a boolean isIpv6";
                return false;
            }

            config.m_allowedIPAddressRanges.append(IPAddress(QHostAddress(address.toString()), range.toInt()));
        }

        // Sort allowed IPs by decreasing prefix length.
        std::sort(config.m_allowedIPAddressRanges.begin(), config.m_allowedIPAddressRanges.end(),
                  [&](const IPAddress &a, const IPAddress &b) -> bool { return a.prefixLength() > b.prefixLength(); });
    }
...
bool Daemon::supportServerSwitching(const InterfaceConfig &config) const
{
    ...
    const InterfaceConfig &current = m_connections.value(config.m_hopType).m_config;

    return current.m_privateKey == config.m_privateKey && current.m_deviceIpv4Address == config.m_deviceIpv4Address
...
bool Daemon::switchServer(const InterfaceConfig &config)
{
    ...
    Q_ASSERT(m_connections.contains(config.m_hopType));
    const InterfaceConfig &lastConfig = m_connections.value(config.m_hopType).m_config;
...
QJsonObject Daemon::getStatus()
{
    ...
    for (const WireguardUtils::PeerStatus &status : peers) {
        ...
        json.insert("serverIpv4Gateway", QJsonValue(connection.m_config.m_serverIpv4Gateway));
        json.insert("deviceIpv4Address", QJsonValue(connection.m_config.m_deviceIpv4Address));
...
void Daemon::checkHandshake()
{
    ...
    if (m_handshakeElapsedTimer.isValid() && m_handshakeElapsedTimer.elapsed() > HANDSHAKE_TIMEOUT_MSEC) {
...
void Daemon::startHandshakeTimer()
{
    m_handshakeElapsedTimer.restart();
    m_handshakeTimer.start(HANDSHAKE_POLL_MSEC);
}
